...
% 論文名稱
\newcommand{\chineseTitle}{結合編碼增強因子分解機與量子退火之矽光子反向設計}
\newcommand{\englishTitle}{Inverse Design of Silicon Photonic Structures\\Using Encoding-Enhanced Factorization Machines and Quantum Annealing}
% 論文名稱太長的話, 可以修改covers/inside_var.tex 的顯示語法, 詳情寫在那邊

% 自己的名字 (英文名字有兩種寫法, 一個是姓在後, 一個是放前面還有逗號. )
\newcommand{\studentCnName}{蔡祐昕}
\newcommand{\studentEnName}{Yu Hsin Tsai} % 書名頁
\newcommand{\studentEnNameA}{Tsai, Yu Hsin} % 封面用
...
\newcommand{\NameofDepartmentInstituteEN}{Department of Photonics} % 系所名稱
\newcommand{\NameofDegree}{Master of Science} % 學位名稱, 常見的是Master of Science, 不同系所採用不同寫法, 記得查學校的資料.
\newcommand{\ResearchTopic}{Department of Photonics} % 研究領域, 請看下方[說明2]
...
\input{Sections/1.Introduction_Motivation} \newpage
\input{Sections/2.Relatedwork} \newpage
\input{Sections/3.Theory} \newpage
\input{Sections/4.Method_Result} \newpage
